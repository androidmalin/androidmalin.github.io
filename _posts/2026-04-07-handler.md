...
```mermaid
flowchart LR
    A[Handler 核心问题] --> B[主线程消息循环]
    A --> C[消息发送]
    A --> D[消息对象]
    A --> E[延迟与屏障]
    A --> F[内存泄漏]

    B --> B1[Looper.loop 死循环]
    B --> B2[MessageQueue.next 阻塞]
    B --> B3[nativePollOnce 休眠]
    B --> B4[epoll 唤醒机制]

    C --> C1[post 发送 Runnable]
    C --> C2[sendMessage 发送 Message]
    C --> C3[最终都封装成 Message]
    C --> C4[dispatchMessage 分发]

    D --> D1[Message.obtain 复用]
    D --> D2[全局消息池节省资源]

    E --> E1[按 when 时间入队]
    E --> E2[SyncBarrier 同步屏障]
    E --> E3[IdleHandler 空闲回调]

    F --> F1[非静态 Handler 持有 Activity]
    F --> F2[Message 持有 Handler]
    F --> F3[延迟消息导致无法回收]
    F --> F4[解决:静态内部类+弱引用]
```
